...
class LSTMCM(object):
    def __init__(self, config, cell, reuse=None):
        ...
        with tf.device(self.device), tf.name_scope(self.cell_name), \
            ...
            with tf.variable_scope("optimize"):
                ...
                capped_gvs = [(None if grad is None else tf.clip_by_value(grad, -self.clip_value, self.clip_value), var)
                              for grad, var in gvs]
    ...
    def run(self, sess, data, accuracys):
        ...
        for batch_id in range(num_batch):
            ...
            batch_xs = reshape_perm(batch_xs)
...
if __name__ == "__main__":
    ...
    with tf.Session(graph=graph, config=sess_cfg) as sess:
        ...
        if config.cell_name == 'arnn':
            fname = "pmnist_arnn_k%d.pkl" % (config.K)
        else:
            fname = "pmnist_%s.pkl" % (config.cell_name)
        file_name = os.path.join("result", "pmnist", fname)
